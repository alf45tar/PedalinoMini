...
char foot_char(byte footswitch)
{
  ...
  if (!IS_SINGLE_PRESS_ENABLED(pedals[footswitch].pressMode)) return ' ';
...
void screen_update(bool force = false) {
  ...
  if (!powersaver) {
    ...
    if (lastUsedPedal >= 0 && lastUsedPedal < PEDALS) {
      //strncpy(&buf[strlen(buf)], &bar2[0], map2(pedals[lastUsedPedal].pedalValue[0], 0, MIDI_RESOLUTION - 1, 0, 10));
      //strncpy(&buf[strlen(buf)], "          ", 10 - map2(pedals[lastUsedPedal].pedalValue[0], 0, MIDI_RESOLUTION - 1, 0, 10));
      f = map2(pedals[lastUsedPedal].pedalValue[0], 0, ADC_RESOLUTION - 1, 0, 50);
      p = f % 5;
      f = f / 5;
